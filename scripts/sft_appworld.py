...
from appworld_env import format_task_dataset
...
# remove parameters with None value from dataset 
def clean_nones(obj):
    if isinstance(obj, dict):
        return {k: clean_nones(v) if v is not None else '' for k, v in obj.items()}
    elif isinstance(obj, list):
        return [clean_nones(item) for item in obj]
    else:
        return obj

def process_row(row):
    row['messages'] = clean_nones(row['messages'][1:])  # Slicing here if you want
    return row
...
def main(args):
    ...
    # sft trajectories from appworld runs
    dataset_4 = load_dataset("ai2-adapt-dev/toolu-appworld-sft-a4v1")
    dataset_3 = load_dataset("ai2-adapt-dev/toolu-appworld-sft-a3v1")
    dataset_2 = load_dataset("ai2-adapt-dev/toolu-appworld-sft-a2v2")
    dataset = concatenate_datasets([dataset_4['train'], dataset_3["train"], dataset_2['train']])
    dataset = dataset.map(process_row)

    world = AppWorld(task_id='692c77d_2',  load_ground_truth=True, ground_truth_mode='full')
    tools = world.task.api_docs.function_calling()
    tools = clean_nones(tools)

    print("Starting tokenization")
    tok_counts = []
    for row in dataset:
        toks = tokenizer.apply_chat_template(row['messages'], tools=tools, add_generation_prompt=True, tokenize=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-grad-norm", "-mg", type=float, default=0.1)
